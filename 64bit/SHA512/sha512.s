...
mainAsm:
    ...
    str x0, [x2]				/*x0=address(input)->addr_input*/
    ldr x2, addr_input_length
    str x1, [x2]				/*x1=length(input)->input_length*/
...
startPadding:
	...
	add x22, x20,#1							/*x22<- length in byte of input + 0b10000000*/
	
	add x22, xzr, x22, lsr #DW_SHIFT		/*x22<- index of the dword that we have to modify (from left to right)*/
...
fillEnd:
	...
	lsr x22, x20, #29							/*X10<-3 most significant bits of input_length*8(bits)*/
...
processChunks:
	...
	bl prdDwords
	mov x21, x0							/*x21=number of dwords*/
...
/*prdDwords
* OUTPUT:
* x0=length in dwords of the processed input (a multiple of 16 dwords - 128  bytes - 1024 bits)	
*/ 
prdDwords:
...
/*						
 * copyProcessed
 * Copy in the first 16 dwords of message the dwords of chunck
 *
 * PARAM: x0=chunk offset in processed input
*/		
copyProcessed:	
	...
	add x0, x0, x22						/*x0=chunk address*/	
			
	mov x20,#0							/*x20 counter*/

loop_pCh: 
	ldr x22,[x0,x20, lsl #DW_SHIFT]		/*x22<-chunk[i]*/
	str x22,[x21,x20, lsl #DW_SHIFT]	/*message[i]=chunck[i]*/	
	add x20, x20,#1
	cmp x20, #16
	bne loop_pCh
...
loop_fm:	
	...
	ldr x23, [x21, x22, lsl #DW_SHIFT]	/*X11 <- w[i-15]*/		
			
	/*x24=s0*/
	ror x24, x23, #1					/*x24=ror(w[i-15],1)*/	
	ror x25, x23, #8					/*x25=ror(w[i-15],8)*/	
	eor x24, x24,x25					/*x24=x24 XOR x25*/
	lsr x25, x23, #7					/*x25=lsr(w[i-15],7)*/
	eor x24,x24, x25					/*x24=x24 XOR x25*/

	/*x23 <-w[i-2]*/
	sub x22, x20, #2					/*x22=i-2*/
	ldr x23, [x21, x22, lsl #DW_SHIFT]	/*x23 <-w[i-2]*/
			
	/*x25=s1*/
	ror x25, x23, #19					/*x25=ror(w[i-2],19)*/
	ror x26, x23, #61					/*x26=ror(w[i-2],61)*/	
	eor x25, x25, x26					/*x25=x25 XOR x26*/
	lsr x26, x23, #6					/*x26=lsr(w[i-2],6)*/
	eor x25, x25, x26					/*x25=x25 XOR x26*/			
			
	add x24, x24, x25					/*x24=s0+s1*/
			
	/*x23 <- w[i-16]*/
	sub x22, x20, #16					/*x22= i-16*/
	ldr x23, [x21, x22, lsl #DW_SHIFT]	/*x23 <- w[i-16]*/
			
	add x24, x24, x23					/*x24=s0+s1+w[i-16]*/
			
	/*x23 <- w[i-7]*/
	sub x22, x20, #7					/*x22= i-7*/
	ldr x23, [x21, x22, lsl #DW_SHIFT]	/*x23 <- w[i-7]*/			
										
	add x24, x24, x23					/*x24=s0+s1+w[i-16]+w[i-7]*/
			
	str x24, [x21, x20, lsl #DW_SHIFT]	/*w[i]=s0+s1+w[i-16]+w[i-7]*/	
bp:		
	/* counter update*/
	add x20, x20, #1					/*i++*/
	cmp x20, #80					
...
initLetter:
	...
	str x20, [x0]						/*letter<-hash[i]*/

	ldp x20, x21, [sp], #16				/*push {lr, ...} $ */
...
compress:
	...
	mov x20, #0							/*x20 counter*/
loop_cpr:	
	mov x0, x20							/*input for make_temp1*/
...
makeTemp1:
	...
	/*x21 := (e rightrotate 14) xor (e rightrotate 18) xor (e rightrotate 41)*/
	ldr x26, addr_e
	ldr x26, [x26]						/*x26<-e*/
...
makeTemp2:	 
	...
	/*x21 := (a rightrotate 28) xor (a rightrotate 34) xor (a rightrotate 39)*/
	ldr x20, addr_a				
	ldr x20, [x20]						/*x20<-a*/
	...
	/*x22 := (a and b) xor (a and c) xor (b and c)*/
	ldr x23, addr_b
	ldr x25, addr_c
	ldr x23, [x23]						/*x23<-b*/
	ldr x25, [x25]						/*x25<-c*/
	...
	add x0, x21, x22					/*x0=S0+maj, output*/
